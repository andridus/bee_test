defmodule BeeTest.BasicApiTest do
  ...
  describe "Basic Api" do
    ...
    test "all" do
      ...
      ### test filters
      assert [user] =
               User.Api.all(where: [function: {:in, [:user]}], select: [:username], limit: 1)
    ...
    test "query relation" do
      ...
      assert {:ok, %{id: _, comments: [], posts: [%{id: _, comments: [%{id: _}]}]}} =
               User.Api.get(uid,
                 ...
                 preload: [
                   posts: [preload: [:comments]],
                   comments: [where: [status: {:eq, :PENDING}]]
                 ]
               )
      ...
      assert {:ok, %{id: _, comments: [%{id: _}], posts: [%{id: _, comments: [%{id: _}]}]}} =
               User.Api.get(uid,
                 ...
                 preload: [
                   posts: [preload: [:comments]],
                   comments: [where: [status: {:eq, :APPROVED}]]
                 ]
               )
  ...
  defp create_text(words, sep \\ " "),
    do: for(_w <- 0..words, do: Bee.unique(:rand.uniform(25))) |> Enum.join(sep)
end
